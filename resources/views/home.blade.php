...
<div id="mobile-nav" class="fixed top-0 right-0 w-[280px] h-full bg-bg-dark2 border-l border-border-dark z-[201] transform translate-x-full transition-transform duration-300 ease-in-out overflow-y-auto">
  <div class="p-5">
    ...
    <div class="flex flex-col gap-2">
      ...
      <a href="{{ route('rankings') }}" class="mobile-nav-link font-heading text-base font-semibold tracking-[0.8px] uppercase text-muted px-3 py-2.5 rounded-md transition-all duration-150 no-underline hover:text-text-light hover:bg-bg-dark3">Power Rankings</a>
      ...
      <a href="{{ route('fantasy.index') }}" class="mobile-nav-link font-heading text-base font-semibold tracking-[0.8px] uppercase text-gold px-3 py-2.5 rounded-md transition-all duration-150 no-underline hover:text-text-light hover:bg-bg-dark3">Fantasy Hub</a>
...
<nav id="main-nav" class="fixed top-0 left-0 right-0 z-[100] h-[60px] flex items-center justify-between px-4 sm:px-6 md:px-10 gap-0 transition-all duration-300 border-b border-transparent">
  ...
  <div class="hidden lg:flex items-center gap-0 ml-6">
    ...
    <a href="{{ route('fantasy.index') }}" class="nav-link font-heading text-xs font-semibold tracking-[0.8px] uppercase text-gold font-bold px-2.5 py-1.5 rounded-md transition-all duration-150 no-underline hover:text-text-light">Fantasy Hub</a>
    <a href="{{ route('rankings') }}" class="nav-link font-heading text-xs font-semibold tracking-[0.8px] uppercase text-muted px-2.5 py-1.5 rounded-md transition-all duration-150 no-underline hover:text-text-light">Power Rankings</a>
  ...
  <div class="flex items-center gap-2">
    <!-- Search Bar -->
    <div class="hidden lg:flex items-center bg-bg-dark3 border border-border-dark rounded-md px-2 py-1 mr-4">
      <form action="{{ route('search') }}" method="GET" class="flex items-center">
        <input type="text" name="q" placeholder="Search teams or players..." class="bg-transparent text-[10px] text-text-light outline-none w-48 placeholder-muted">
        <button type="submit">
          <svg class="w-3.5 h-3.5 text-muted hover:text-gold transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
        </button>
      </form>
    </div>
  ...
  document.getElementById('matches-grid').innerHTML = data.map(m => {
    ...
    <div class="bg-bg-dark3 border border-border-dark rounded-xl overflow-hidden transition-all duration-200 hover:border-border-dark2 cursor-pointer" onclick="window.location.href='/matches/${m.id}'">
      ...
      <div class="px-2.5 sm:px-3.5 py-3 sm:py-4">
        ...
        ${isUpcoming ? `
          <div class="mt-4 pt-3 border-t border-border-dark" onclick="event.stopPropagation()">
